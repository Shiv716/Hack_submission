...
#     docs = retriever.invoke(state["description"])
...
def meme_node(state):
    try:
        ...
        print("❌ Error in meme_node:", e)
        ideas = [{"idea": "Parse error", "audio_script": raw[:80] if 'raw' in locals() else "", "video_prompt": "N/A"}]
